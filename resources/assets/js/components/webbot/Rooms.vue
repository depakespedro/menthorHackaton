            ...
            <div v-show="!isIframe || widgetOpen" class="webbot-container" :class="(isIframe) ? 'in-iframe' : ''" :style="widgetStyles">
                <div class="webbot-wrapper">
                    <div class="webbot-header" v-bind:style="{ 'background': bgColor }">
                        <div class="head-text">Комната {{ this.roomId }}</div>
                            ...
                            <ul v-if="messages.length" class="messages-list">
                                <li v-for="message in messages" class="message clearfix">
                                    <div class="media-body" :class="(message.type === 'text-my') ? 'message-my' : 'message-bot'">
                                        ...
                                        <div class="message-text" v-if="message.text || message.type === 'action_typing'" v-bind:style="{ 'background': getBgColor(message) }">
                                            ...
                                            <div class='message-tail'>
                                                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="25" height="24" viewBox="0 0 25 24">
                                                    <g><path v-bind:style="{ 'fill': getBgColor(message) }" d="M19,2a11.921,11.921,0,0,1-6.86,7.5c-5.8,2.317-8.38,1.5-8.38,1.5l16,6"/></g>
                                                </svg>
                                            </div>
                                        </div>
                                    </div>
...
<script>
    ...
    export default {
        ...
        props: ['roomId', 'projectId', 'widgetId', 'surveyId', 'widgetJson', 'startData'],
        computed: {
        ...
        methods: {
            ...
            getConversationId (result) {       
                return new Promise((resolve, reject) => {
                    ...
                    conversationId = CryptoJS.SHA1(this.roomId + this.surveyId).toString();
            ...
            sendMessage (event, button = null, text=null) {
                ...
                window.console.log('before send message: ', message);
            ...
            getBgColor(message) {
                if (message.type !== 'text-my') {
                    return '#f0efef';
                }

                if (this.respondent && message.respondent_id && parseInt(message.respondent_id) !== parseInt(this.respondent.id)) {
                    return '#f0efef';
                }
            ...
            setBotMessage (message) {
                ...
                if (this.isIframe && (message.type === 'trigger' || 
                    ...
                    window.console.log('trigger message: ', message);
                ...
                return new Promise((resolve, reject) => {
                    ...
                    if (message.respondent_id) {
                        newMessage.respondent_id = message.respondent_id;
                    }
        ...
        created () {
            ...
            this.getFingerprint().then(({result, components}) => {
                ...
                this.getConversationId(result).then((conversationId) => {
                    ...
                    this.Echo.join('Webbot.Room.' + this.roomId + '.' + this.surveyId)
                        .here((users, data) => {
                            users.forEach((user) => {

                                // Находим текущего пользователя по отпечатку
                                if (!this.respondent && user.fp === result) {
                                    window.console.log('conversation: ', user);

                                    this.respondent = user;
                                    this.respondentsNames = user.respondents_names;
                            ...
                            let respondentName = this.respondentsNames[user.id] || null;
                            let textBot = respondentName ? respondentName + ' присоединился к беседе' : 'Собеседник присоединился к чату';

                            this.messages.push({text: textBot, type: 'text-bot', buttons: null});
                            setTimeout(() => {
                                this.scrollDown();
                            }, 40);
                        }).leaving((user) => {
                            let respondentName = this.respondentsNames[user.id] || null;
                            let textBot = respondentName ? respondentName + ' покинул(а) беседу' : 'Собеседник покинул чат';
